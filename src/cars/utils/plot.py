...
def plot_global_explanation(
    results_dir: Path, dataset_name: str, concept_categories: dict = None
) -> None:
    ...
    for class_idx, concept, method in itertools.product(classes, concepts, methods):
        attr = np.array(
            metrics_df.loc[
                (metrics_df.Class == class_idx) & (metrics_df.Method == method)
            ][concept]
        )
    ...
    logging.info(
        f"TCAR-True Prop. Correlation: {np.corrcoef(tcar_scores, true_scores)[0, 1]:.2g}"
    )
    if "TCAR Sensitivity" in methods:
        tcar_sensitivity_scores = plot_df.loc[plot_df.Method == "TCAR Sensitivity"][
            "Score"
        ]
        logging.info(
            f"TCAR_Sensitivity-True Prop. Correlation: {np.corrcoef(tcar_sensitivity_scores, true_scores)[0, 1]:.2g}"
        )
    logging.info(
        f"TCAV-True Prop. Correlation: {np.corrcoef(tcav_scores, true_scores)[0, 1]:.2g}"
    )
    if concept_categories is not None:
        for class_idx, concept_category in itertools.product(
            classes, concept_categories
        ):
            save_dir = results_dir / concept_category.lower().replace(" ", "-")
            if not save_dir.exists():
                os.makedirs(save_dir)
            filtered_concepts = concept_categories[
                concept_category
            ]  # Use only the concept in the given category
            ax = sns.barplot(
                data=plot_df[
                    (plot_df.Class == class_idx)
                    & (plot_df.Concept.isin(filtered_concepts))
                ],
                x="Concept",
                y="Score",
                hue="Method",
            ...
            plt.savefig(
                save_dir
                / f"{dataset_name}_global_category_{concept_category}_class_{class_idx}.pdf"
            )
            plt.close()
    else:
        for class_idx in classes:
            ax = sns.barplot(
                data=plot_df.loc[plot_df.Class == class_idx],
                x="Concept",
                y="Score",
                hue="Method",
            )
...
def plot_seer_global_explanation(results_dir: Path) -> None:
    ...
    plot_df = pd.DataFrame(
        plot_data, columns=["Method", "Patient outcome", "Concept", "Score"]
    )
...
def plot_attribution_correlation(
    results_dir: Path,
    dataset_name: str,
    filtered_concepts: list = None,
    ...
    if filtered_concepts is not None:
        attribution_dic = {
            concept_name: attribution_dic[concept_name]
...
def plot_time_series_saliency(
    ...
    for ax_id, example_id in enumerate(plot_indices):
        ...
        scatter = ax.scatter(
            x=list(range(T)),
            y=tseries[example_id].flatten(),
            cmap=sns.diverging_palette(10, 133, as_cmap=True),
            c=sub_saliency.flatten(),
            vmin=-max_value,
            vmax=max_value,
        )
        ax.set_xlabel("Time")
...
def plot_concept_size_impact(results_dir: Path, dataset_name: str) -> None:
    ...
    metrics_df = pd.read_csv(results_dir / "metrics.csv")
    sns.lineplot(
        data=metrics_df, x="Concept Sets Size", y="Test Accuracy", hue="Concept"
    )
...
def plot_tcar_inter_concepts(results_dir: Path, dataset_name: str) -> None:
    ...
    for i in range(1, n_concepts):
        for j in range(i):
            tcar_matrix[i, j] = jaccard_score(
                metrics_df[concepts[i]], metrics_df[concepts[j]]
            )
...
if __name__ == "__main__":
    ...
    if args.dataset == "cub":
        train_set = CUBDataset(
            [str(Path.cwd() / "data/cub/CUB_processed/class_attr_data_10/train.pkl")],
            use_attr=True,
            no_img=False,
    ...
    if args.name == "concept_accuracy":
        ...
        if args.dataset != "seer":
            plot_global_explanation(
                save_path, args.dataset, concept_categories=concept_categories
            )
        else:
